...
// Attestation key + certificate are generated per device (CryptoP256::attestation).
...
void Ctap2Engine::handleMakeCredential(uint32_t cid, CborDecoder& dec) {
    ...
    const uint8_t *attKey, *attCert;
    size_t attCertLen;
    uint8_t sigDer[80];
    size_t sigLen = sizeof(sigDer);
    if (!cryptoP256.attestation(TKEY_AAGUID, &attKey, &attCert, &attCertLen) ||
        !cryptoP256.signDigest(attKey, digest, sigDer, &sigLen) || sigLen == 0) {
    ...
    respEnc.encodeBytes(attCert, attCertLen);
...
void Ctap2Engine::handleCtap1Msg(uint32_t cid, const uint8_t* req, uint16_t reqLen) {
    ...
    if (ins == 0x01) {
        ...
        size_t derSigLen = sizeof(derSig);
        if (!cryptoP256.attestation(TKEY_AAGUID, &attKey, &attCert, &attCertLen) ||
            !cryptoP256.signDigest(attKey, sigHash, derSig, &derSigLen)) {
            uint8_t errResp[] = {0x6F, 0x00};   // SW_UNKNOWN
            ctapHid.sendResponse(cid, CTAPHID_CMD_MSG, errResp, 2);
            return;
        }
        ...
        rOffset += 32;
        memcpy(respBuf + rOffset, attCert, attCertLen);
        rOffset += attCertLen;
