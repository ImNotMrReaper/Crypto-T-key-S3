...
#define MBEDTLS_GET_UINT32_LE(data, offset) \
    ((uint32_t)((const uint8_t*)(data))[(offset) + 0] | \
    ((uint32_t)((const uint8_t*)(data))[(offset) + 1] << 8) | \
    ((uint32_t)((const uint8_t*)(data))[(offset) + 2] << 16) | \
    ((uint32_t)((const uint8_t*)(data))[(offset) + 3] << 24))
#endif

#ifndef MBEDTLS_PUT_UINT32_LE
#define MBEDTLS_PUT_UINT32_LE(val, data, offset) \
    do { \
        ((uint8_t*)(data))[(offset) + 0] = (uint8_t)((val)); \
        ((uint8_t*)(data))[(offset) + 1] = (uint8_t)((val) >> 8); \
        ((uint8_t*)(data))[(offset) + 2] = (uint8_t)((val) >> 16); \
        ((uint8_t*)(data))[(offset) + 3] = (uint8_t)((val) >> 24); \
    } while(0)
#endif
